      ...
      {config.sections.map((section) => (
        <article key={section.id} className="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm dark:border-slate-800 dark:bg-slate-950 md:p-10">
          <div className="mb-6 inline-flex h-12 w-12 items-center justify-center rounded-xl bg-slate-100 dark:bg-slate-900">
            <section.icon className="h-6 w-6 text-slate-900 dark:text-white" />
          </div>
...
function FAQCard({ title, items }: { title: string; items: { question: string; answer: string }[] }) {
  return (
    <article className="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm dark:border-slate-800 dark:bg-slate-950 md:p-10">
      ...
      <Accordion type="single" collapsible className="mt-8">
        {items.map((item, index) => (
          <AccordionItem key={item.question} value={`faq-${index}`} className="border-slate-200 dark:border-slate-800">
            <AccordionTrigger className="text-left text-lg font-medium hover:no-underline">{item.question}</AccordionTrigger>
            <AccordionContent className="text-base leading-relaxed text-slate-600 dark:text-slate-400">{item.answer}</AccordionContent>
          </AccordionItem>
        ))}
      </Accordion>
    </article>
...
function WebpConverterPage({ config }: { config: ToolPageConfig }) {
  ...
  return (
    <PageShell
      ...
      description={config.description}
      path={config.path}
      accent="bg-slate-50 dark:bg-[#0a0a0a]"
      schema={schema}
    >
      <HeroIntro
        icon={FileArchive}
        badge="WEBP Converter"
        title="WebP Converter for Modern Web"
        body="Convert your images to WebP format instantly. Designed for developers and SEO professionals who care about page speed, Core Web Vitals, and clean user experiences."
        badgeClass="border-slate-200 bg-white text-slate-900 dark:border-slate-800 dark:bg-slate-950 dark:text-white"
      />
      ...
      <FAQCard title="Questions people ask about WebP conversion" items={config.faq} />
    </PageShell>
  );
}
...
function ImageCompressorPage({ config }: { config: ToolPageConfig }) {
  ...
  return (
    <PageShell
      ...
      description={config.description}
      path={config.path}
      accent="bg-[linear-gradient(180deg,#F0F4FF,rgba(255,255,255,0.98))] dark:bg-[linear-gradient(180deg,rgba(17,24,39,0.8),rgba(2,6,23,0.98))]"
      schema={schema}
    >
      <HeroIntro
        icon={Compass}
        ...
        body="Reduce image file size in your browser with quality controls designed for practical web publishing."
        badgeClass="border-[#6B53FF]/20 bg-[#6B53FF]/10 text-[#6B53FF] dark:border-[#6B53FF]/50 dark:bg-[#6B53FF]/20 dark:text-[#A36BFF]"
      />
      ...
      <FAQCard title="Questions people ask about image compression" items={config.faq} />
    </PageShell>
  );
}
...
function GeoTaggerPage({ config }: { config: ToolPageConfig }) {
  ...
  return (
    <PageShell
      ...
      description={config.description}
      path={config.path}
      accent="bg-[linear-gradient(180deg,rgba(255,247,237,0.84),rgba(255,255,255,0.98))] dark:bg-[linear-gradient(180deg,rgba(124,45,18,0.20),rgba(2,6,23,0.98))]"
      schema={schema}
    >
      <HeroIntro
        icon={MapPinned}
        badge="Geo Tagger"
        title="Free Geo Tagger for Local SEO Images"
        body="Use this geo tagger to add GPS metadata to images directly in the browser. Pin the exact location, embed latitude and longitude, and export geo-tagged photos for local SEO workflows, Google Business Profile assets, and location-specific content."
        badgeClass="border-amber-300/60 bg-amber-50/80 text-amber-700 dark:border-amber-700/50 dark:bg-amber-950/30 dark:text-amber-300"
      />
      ...
      <FAQCard title="Questions people ask about geo tagging" items={config.faq} />
    </PageShell>
  );
}
...
function DefaultToolPage({ config }: { config: ToolPageConfig }) {
  return (
    <div className="flex min-h-screen flex-col bg-background">
      <SeoHead title={config.metaTitle} description={config.description} path={config.path} />
      <Navbar />
      <main className="flex-1">
        <section className="relative overflow-hidden px-4 pb-16 pt-12 md:pt-20">
          <div className={`pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(255,255,255,0.95),transparent_24%)] dark:bg-[radial-gradient(circle_at_top_left,rgba(255,255,255,0.06),transparent_24%)]`} />
          <div className={`pointer-events-none absolute inset-x-0 top-0 h-[34rem] bg-gradient-to-br ${config.heroGradient}`} />
          <div className="container relative mx-auto">
            <div className="space-y-8">
              <div className="max-w-3xl">
                <span className={`inline-flex items-center gap-2 rounded-full border px-4 py-1.5 text-[11px] font-extrabold uppercase tracking-[0.2em] ${config.accentClass}`}>
                  <config.icon className="h-3.5 w-3.5" />
                  {config.heroEyebrow}
                </span>
                <h1 className="mt-6 max-w-3xl text-4xl font-black tracking-tight md:text-5xl lg:text-6xl">{config.heroTitle}</h1>
                <p className="mt-5 max-w-2xl text-base leading-8 text-muted-foreground md:text-lg">{config.heroDescription}</p>
              </div>

              <div id="tool">
                <ToolSection forcedMode={config.mode} routeMode />
              </div>

              <RichCard
                eyebrow="SEO CONTENT"
                title={`A dedicated ${config.title.toLowerCase()} page`}
                body="This page uses a single-column layout so the tool, supporting content, and FAQ read like one focused landing page instead of a split dashboard. That structure usually works better for search intent, readability, and user flow."
              >
                <BulletRows items={config.featureBullets} />
              </RichCard>
              ...
              <FAQCard title={`Questions people ask about ${config.shortLabel.toLowerCase()}`} items={config.faq} />
            </div>
          </div>
        </section>
      </main>
...
export function ToolPage({ config }: { config: ToolPageConfig }) {
  if (config.mode === "webp-converter") return <WebpConverterPage config={config} />;
  if (config.mode === "online-image-compressor") return <ImageCompressorPage config={config} />;
